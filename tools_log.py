...
class GwLogger(object):
    ...
    def __init__(self, log_name, log_level, log_suffix, folder_has_tstamp=False, file_has_tstamp=True,
        ...
        self.tab_python = f"{lib_vars.plugin_name.capitalize()} PY"
        self.tab_db = f"{lib_vars.plugin_name.capitalize()} DB"

        # Define log folder in users folder
        main_folder = os.path.join(tools_os.get_datadir(), lib_vars.user_folder_dir)
...
def log_debug(text=None, context_name="giswater", parameter=None, logger_file=True, stack_level_increase=0,
    ...
    msg = _qgis_log_message(text, Qgis.Info, context_name, parameter, tab_name, msg_params)
...
def log_warning(text=None, context_name="giswater", parameter=None, logger_file=True, stack_level_increase=0,
    ...
    msg = _qgis_log_message(text, Qgis.Warning, context_name, parameter, tab_name, msg_params)
    if lib_vars.logger and logger_file:
...
def log_error(text=None, context_name="giswater", parameter=None, logger_file=True, stack_level_increase=0,
    ...
    msg = _qgis_log_message(text, Qgis.Critical, context_name, parameter, tab_name, msg_params)
    if lib_vars.logger and logger_file:
        lib_vars.logger.error(msg, stack_level_increase=stack_level_increase)


def log_db(text=None, color="black", bold='', header="SERVER EXECUTION", message_level=Qgis.Info, logger_file=True,
...
def _qgis_log_message(text=None, message_level=Qgis.Info, context_name="giswater", parameter=None, tab_name=None,
                      msg_params=None):
    """
    ...
    if tab_name is None and lib_vars.logger:
        tab_name = lib_vars.logger.tab_python
